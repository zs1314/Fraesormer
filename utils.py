...
class EarlyStopping:
    def __init__(self, patience=10, min_delta=0.0):
        """
        初始化早停策略。
        :param patience: 允许精度不提升的最大epoch数，超过该值则终止训练。
        :param min_delta: 精度提升的最小变化幅度，小于该值视为无提升。
        """
    ...
    def __call__(self, acc1):
        """
        检查当前验证集的精度，如果没有显著提升，增加计数，达到耐心阈值则触发早停。
        :param acc1: 当前epoch的验证集精度。
        """
        if self.best_acc is None:
            self.best_acc = acc1
        elif acc1 - self.best_acc < self.min_delta:
            self.counter += 1
            if self.counter >= self.patience:
                self.early_stop = True
            print("early stop epoch {}".format(self.counter))
